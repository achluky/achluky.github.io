...
\begin{frame}{Marks: Elemen Geometri Dasar}
    ...
    \begin{itemize}
        ...
        \vspace{0.2cm}
        \item \textbf{1D (Line/Garis):} Memiliki panjang tetapi lebar diabaikan. Menunjukkan koneksi/lintasan. \textit{(Contoh: Garis tren pada line chart)}, garis penghubung pada Network Graph.
        \vspace{0.2cm}
        \item \textbf{2D (Area/Bidang):} Memiliki panjang dan lebar (luasan) yang mengisi ruang. \textit{(Contoh: Poligon batas provinsi pada peta)}, Irisan pada Pie Chart atau batas negara pada Peta
    \end{itemize}
\end{frame}
...
\begin{frame}{Definitions: Marks and channels}
	\begin{itemize}
		\item Marks
		\item Channels
		\item Channel properties differ
...
\begin{frame}{Dua Prinsip (\textit{The Golden Rules})}
...
\begin{frame}{Dua Prinsip Desain (The Golden Rules)}
	...
	\begin{itemize}
        \item Contoh
        \begin{itemize}
			\item Menggunakan panjang garis (Magnitude) untuk menunjukkan perbedaan kategori (Identity). Orang akan mengira kategori dengan garis lebih panjang lebih penting, padahal itu cuma nama. 
            \item Jika membandingkan "Total Penjualan" adalah tugas utama (Task), gunakan Channel Posisi/Panjang (seperti Bar Chart). Jangan gunakan gradasi warna, karena mata manusia sangat buruk dalam menghitung selisih angka murni hanya dari melihat tingkat kegelapan warna.
	    \end{itemize}
	\end{itemize}
\end{frame}


\begin{frame}{Visual encoding}
	\begin{itemize}
		\item analyze idiom structure as combination of marks and channels
		\includegraphics[width=0.9\textwidth]{image7.png}
	\end{itemize}
\end{frame}


\begin{frame}{Redudant encoding}
	\begin{itemize}
		\item multiple channels: sends stronger message but uses up channels
...
\begin{frame}{Contoh}
	\begin{itemize}
		\item Visualisasi Data Cuaca di Berbagai Kota
		\item Bayangkan Anda memiliki data tabel berisi informasi cuaca di beberapa kota besar:
		\begin{itemize}
            \item Kolom: Nama Kota, Suhu (°C), Kelembapan, Kondisi Cuaca (Cerah, Berawan, Hujan)
            \item Baris: Jakarta, 30°C, 70%, Cerah; Bandung, 25°C, 80%, Berawan; Surabaya, 28°C, 75%, Hujan 
            \item Item (Baris Data): Nama Kota (Jakarta, Tokyo, London, Sydney)
            \item Atribut 1 (Kuantitatif): Suhu Udara ($^\circ$C).
            \item Atribut 2 (Kuantitatif): Kelembapan Udara.
            \item Atribut 3 (Kategorikal): Benua (Asia, Eropa, Australia).
        \end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Contoh}
	\begin{itemize}
        \item Mengombinasikan Mark dan Channels untuk menyampaikan informasi secara efektif:
		\begin{itemize}
            \item Mark yang dipilih: Point (0D - Titik). Setiap satu titik di layar mewakili satu kota.
            \item Channel 1 (Horizontal Position / Sumbu X): Dipakai untuk menyandikan Suhu Udara. Semakin ke kanan letak titiknya, semakin panas suhunya.
            \item Channel 2 (Vertical Position / Sumbu Y): Dipakai untuk menyandikan Kelembapan Udara. Semakin ke atas letak titiknya, semakin lembap udaranya.
            \item Channel 3 (Color Hue / Rona Warna): Dipakai untuk menyandikan Benua. Titik berwarna Merah untuk Asia, Biru untuk Eropa, dan Hijau untuk Australia.
        \end{itemize}
	\end{itemize}
\end{frame}
